...
const dataPath = path.join(process.cwd(), "public", "interventions-data.json");
const modPath = path.join(process.cwd(), "public", "interventions-modifications.json");
...
async function loadData() {
  try {
    ...
    console.error("Failed to load interventions data, using empty array:", err.message);
    return [];
  }
}
...
async function syncLocalDataFile(mod: any) {
  try {
    ...
    console.error("Failed to sync interventions-data.json:", err);
  }
}
